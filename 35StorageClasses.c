/*  Storage Classes In C Auto, Extern Static & Register Storage Classes : #49

"A storage class defines scope, default initial value, and a lifetime of a variable."
Here, scope refers to the variable's availability at places.
The initial default value refers to the value present in the variables as default before being initialized, and the lifetime refers to the variable's duration of life.
...
*** Extern Keyword:
Using the extern keyword, we inform our compiler that the variable is already declared at some other place. By doing so, we can use the same variable with the same space, without allocating its new memory and accessing the same variable in some other file.
Its syntax is simple as we have to use the extern keyword, and it will automatically access it from the other file.
...
//  Code as described/written in the Harry's video:-                         // video no. 49

#include <stdio.h>                             // ek baar video phir se dekho doubt hai code me
// #include "temp.c"
